...
-- メニュー
-------------------------------------------
function btn.newMenu(option)
	-----------------------
	-- sample
	--
	-- option = {{str="違反法", action=function() print("----") end}}
	-- local menu, show, close = btn.newMenu(option)
	-- 
	...
	local blackFilter = createBlackFilter(0, 0, _W, _H, group)
	blackFilter.alpha = 0
	...
	local function show()
		transition.to(btnGroup,   {time=230, y = _H - (#option*90 + 110)})
		transition.to(blackFilter,{time=230, alpha = 1})
	end

	-- 閉じる関数
	local function close()
		tsprint("btn.lua  close()")
		transition.to(blackFilter,{time=130, alpha = 0})
		transition.to(btnGroup, {time=130, y = btnGroup.y + #option*90 + 110, onComplete=
			function()
				tsprint("close( )=========================")
	...
	for k, v in pairs(option) do
		...
		if #option == 1 then
			-- メニューが１つしかないとき
			image = display.newImage(btnGroup, imageDir.."image/90pxBnt.png", 0, 0)
		else
			-- メニューの一番上
			if k == 1 then
				
				image = display.newImage(btnGroup, imageDir.."image/90px.png", 0, 0)
				local line  = display.newLine(btnGroup, 0, image.y + 45 - 1, 608, image.y + 45 - 1)
				line:setColor(225)
			...
			elseif k == #option then
				image = display.newImage(btnGroup, imageDir.."image/90px.png", 0, 0)
				image.rotation = 180
				image.x = 0 + image.width*0.5 
				image.y = 0 - image.height*0.5 + 90 * k

			else
				image = display.newRect(btnGroup ,0, 90*(k-1), 608, 90)
				local line  = display.newLine(btnGroup, 0, image.y + 45 - 1, 608, image.y + 45 - 1)
				line:setColor(225)				
			end
		end
		...
		-- ボタン上のテキスト
		local text  = display.newText(btnGroup, v.str, 0, 0, nil, v.fontSize or 36)
		if v.fontColor ~= nil then
			text:setTextColor( v.fontColor[1], v.fontColor[2], v.fontColor[3], v.fontColor[4] or 255)
		else
			text:setTextColor(rgbSet.rgbFunction(1)[1],rgbSet.rgbFunction(1)[2],rgbSet.rgbFunction(1)[3])
		end
		text.x, text.y = image.x, image.y
		...
		if v.action ~= nil then
			...
			image:addEventListener( "touch", 
				function(event)
					if event.phase == "began" then
						image:setFillColor( 224,225,225)
						timer.performWithDelay(230, 
							function() 
								if pcall(function ()
										if image then
									...
									tsprint("btn.lua: btn.newMenu()  pcall        exception handling")
									display.remove(image)
									image = nil
								end	
							end
	...
	----------------------------
	--閉じるボタン
	local closeBtn = display.newImage(btnGroup ,imageDir.."image/90pxBnt.png", 0, 90 * #option + 12)

	-- 閉じるボタン上のテキスト
	local closeText  = display.newText(btnGroup, translations["Cancel"][_isLanguage], 0, 0, nil, 36)
	closeText:setTextColor(136,153,166)	
	closeText.x, closeText.y = closeBtn.x, closeBtn.y
	
	-- 閉じるボタンを押した時の動作
	closeBtn:addEventListener( "touch", 
		function(event)
			if event.phase == "began" then
				closeBtn:setFillColor(224,225,225)
				timer.performWithDelay(230, 
				    function()
						if pcall(function ()
								if closeBtn then
									closeBtn:setFillColor(255)
								else
	...
	blackFilter:addEventListener("tap", 
		function(event)
			tsprint("btn.newMenu()    blackFilter")
			-- close()
			backLibrary.action()
			return true
		end)
	...
	backLibrary.save({
		action = function()
			tsprint("btn.newMenu()    backLibrary.save")
			close()
			return true
		end,
	})

	return group, show, close
end
